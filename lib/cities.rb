...
class Cities
  ...
  def get_cities_list(city_name)
    ...
    cities_list = cities_hash.select do |item|
  ...
  def get_city_coordinates(city_name, city_country)
    ...
    city = cities_hash.select do |item|
      ActiveSupport::Inflector.transliterate(item['name']) == ActiveSupport::Inflector.transliterate(city_name) && item['country'] == city_country
    end
